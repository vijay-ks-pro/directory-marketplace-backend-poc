...
import { TENANT_IDENTIFIER } from '@app/common/constants';
...
export class TenancyCoreModule implements OnApplicationShutdown {
  ...
  private static getTenant(
    ...
    const { tenantIdentifier = TENANT_IDENTIFIER, isTenantFromSubdomain = false } =
      moduleOptions;
  ...
  private static async getConnection(
    ...
    /* ********************************************************** */
    // CONNECTION MAGIC HAPPENS HERE
    /* ********************************************************** */

    const DEFAULT_CONNECTION = 'default';

    // Check if tenantId exist in the connection map
    const exists = connMap.has(DEFAULT_CONNECTION);

    // Return the connection if exist
    if (exists) {
      const connection = connMap.get(DEFAULT_CONNECTION) as Connection;
      ...
      return connection.useDb(tenantId);
    }

    const DB_URL = process.env.MONGO_DB_CLUSTER_URL + tenantId;

    // Otherwise create a new connection
    const connection = createConnection(DB_URL, {
    ...
    connMap.set(DEFAULT_CONNECTION, connection);
